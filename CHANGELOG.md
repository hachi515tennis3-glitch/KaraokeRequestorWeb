...
## つぼはち改良・本家同期（2026-09-20）
- 派生版の最新 `8c30531` を基準に、本家 `04ef839`（PR #222 まで、v0.10.1）を同期。
- クール一覧は本家 PR #220 の実装に統一。表示の有効設定、BS5検索への遷移、一般利用者の強制同期防止を採用。
- 背景画像のタイル表示、部分設定保存時の別部屋URL表示設定の保持、バージョン定義の統一を取り込み。
- ゆかナビ設計書・READMEのアプリ紹介・インストーラーへのAPI追加は同期対象外。既存の独自機能は維持。
- AGENTS.md / CLAUDE.md の重複を整理し、詳細資料を docs/development-reference.md に分離。

## v0.10.1（2026年6月〜8月・安定性向上）
曲情報の修正：曲名・歌手名・作品名などが違っているとき、予約時に修正して予約できる機能を追加（修正内容は予約一覧の表示とリスト出力に反映。ゆかりすたーDBは書き換わりません）。修正ログのCSV出力も追加
年齢制限曲：利用者ごとのオプトイン制に変更。表示していない場合は隠れている曲数を表示
検索の拡張：検索UI v2 にクール（期別）一覧タブを追加。りすたーDBのカテゴリー表示順・非表示を `listerdb_config.ini` で設定可能に
背景画像：タイル状に並べる表示モードを追加。iOS/Androidでの壁紙のずれ・伸縮を修正
外観プリセット：CSS変数ファイル方式に再設計。インストーラーアイコン・ファビコン・マスコット画像を新デザインに更新
操作ミス防止：曲停止・曲終了ボタンに確認ポップアップを追加
安定性の改善：予約送信タイムアウトによる二重予約、SSE接続の滞留によるポート枯渇、NAS I/O停滞時のワーカー占有、キーチェンジ連打時の接続増幅などを対策
不具合修正：シークレット予約の曲情報が他の人の予約一覧へ漏れる問題、予約差し替え時に曲情報が更新されない問題、設定画面で別部屋URLの表示設定が消える問題、途中参加者の予約順、MPC曲別音量オフセットの適用方法 ほか

## v0.10.0-beta（2026年4月〜6月・大規模UIリニューアル）
...
設定バックアップ/リストア機能：設定ファイルやデータをまとめてバックアップ・復元  
