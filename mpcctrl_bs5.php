...
// 音量初期値リセット / 字幕補正の実装 (api/player.php と共用)
require_once 'function_playeradjust.php';
...
if (array_key_exists('cmd', $_REQUEST)) {
    ...
    elseif ($l_cmd === 'reset_volume') { print reset_initial_volume_bs5(); }
    elseif ($l_cmd === 'comp_inc') {
        $new = player_compensation_inc();
        header('Content-Type: application/json');
        echo json_encode(['level' => $new]);
    }
    elseif ($l_cmd === 'comp_dec') {
        $new = player_compensation_dec();
        header('Content-Type: application/json');
        echo json_encode(['level' => $new]);
    }
    elseif ($l_cmd === 'comp_reset') {
        player_compensation_reset();
        header('Content-Type: application/json');
