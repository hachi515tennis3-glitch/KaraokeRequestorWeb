...
必要な節だけ参照する開発資料。実装・既定値は現在のコードを優先する。
...
String values in `config.ini` are stored URL-encoded. Most values remain encoded after loading; selected Google settings are decoded in the loader. Check the relevant read/write path and avoid double encoding or decoding.
